...
def part1(x):
	...
	ring = ss**2 - (ss-2)**2
	minVal = np.min([np.abs(((ss-2)**2 + ring/8 + ring/4*y) -x) for y in range(4)])
...
def part2(x, size):
	...
	while True:
		y[xindex][yindex] = y[xindex-1][yindex-1]+ y[xindex][yindex-1]+ y[xindex-1][yindex]+y[xindex+1][yindex]+y[xindex][yindex+1]+y[xindex+1][yindex+1]+y[xindex-1][yindex+1]+y[xindex+1][yindex-1]
